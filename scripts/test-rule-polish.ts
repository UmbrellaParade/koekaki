...
const cases: Case[] = [
  ...
  {
    // 2026-07-28 に実機（Android）で撮られた画面そのままの入力。
    // 繰り返し単位が13文字あり、以前の12文字上限をすり抜けていた。
    label: '実機で出た羅列（13文字単位・空白入り）を1つにまとめる',
    input:
      '今今今 制度のチェックしてます今 制度のチェックしてます今 制度のチェックしてます今 制度のチェックしてます今 制度のチェックしてます今 制度のチェックしてます今 制度のチェックしてます',
    check: (out) => {
      const count = out.split('制度のチェックしてます').length - 1
      if (count !== 1) return `「制度のチェックしてます」が ${count} 回残っている`
      if (/今今/.test(out)) return '「今今」が残っている'
      return null
    },
  },
  {
