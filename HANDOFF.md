...
npm run typecheck
npm test         # 56件（Web回帰26 + ホットキー5 + 貼り付け10 + デスクトップ15）
...
- 回帰テスト56件（ルール整形18 + 音声認識8 + ホットキープロトコル5 + 貼り付けプロトコル10 + デスクトップ15）
...
1. **デスクトップ常駐版（最優先・段階4までコード実装済み）** — 第8章
...
**ボイスバーの参考画像は依頼主から受領済みです。**
設定画面は依頼主側でも場所が分からなかったため、追加で探してもらう必要はありません。
第5段階では受領画像を基準に、フォーカスを奪わないデスクトップ向け表示へ仕上げます。
