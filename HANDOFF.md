...
### 再現対象（Typeless の実仕様・公式ドキュメントで確認済み）

依頼主が実際に Typeless を使っており、「ほぼ同じものを作ってほしい」という要望。
2026-07-28 に公式ヘルプで確認した仕様は次のとおり。
...
| 挿入前のプレビュー | 無し（整形済みのものがそのまま入る） |

出典: https://www.typeless.com/help/quickstart/first-dictation

ボイスバーの見た目と表示位置は、公開情報からは確定できていない。
**依頼主に実物のスクリーンショットを頼むこと**（録音中のバーと設定画面）。
依頼主は画面共有は許可しなかったが、スクリーンショットは何度も送ってくれている。

### こえかき側で満たすべき挙動

- 右 Alt トグル。押した瞬間に録音開始（体感の遅れを作らない）
- 録音中は小さなバーを常時最前面に出す。**フォーカスは絶対に奪わない**
- 停止したら整形し、元のアプリのカーソル位置に挿入する
- 整形エンジンや辞書は Web 版の設定をそのまま使えるようにする
  （`src/lib/storage.ts` の設定形をそのまま流用してよい）
