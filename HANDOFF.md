...
### 5-8. インストール済み PWA は勝手に更新されない（最重要）

**同じ不具合を3回「直した」のに実機で消えず、原因は結局これでした。**
ホーム画面に追加した PWA は、アプリを完全終了しても Service Worker が
古いコードを配り続けます。Android はアプリを閉じてもプロセスを保持するため、
利用者が「開き直した」と言っていても更新されていません。

対策として入れてあるもの:
- 設定画面の一番下に**ビルド日時を表示**（どの版で試したかを毎回確認できる）
- 「最新版に更新」ボタン（SW とキャッシュを捨てて読み込み直す）
- 画面に戻るたびに `registration.update()`、`controllerchange` で自動再読込

**修正を出したあと、実機で症状が消えないと言われたら、まず版数を確認してもらうこと。**
コードを疑う前にキャッシュを疑ってください。

### 5-9. 認識途中の文字は画面に出さない

Web Speech API の途中経過は、同じ語が並んだり書き換わったりして必ず乱れます。
最終出力が正しくても、途中経過が汚いと利用者は「壊れている」と感じます。
Apple のディクテーションが途中を出さないのはこのためです。
既定では出さず、「声を拾っています」だけ伝える設計にしてあります（設定で変更可）。

### 5-10. PowerShell スクリプトは ASCII で書く
